...
int main() {
    ...
    // Lecture des données du formulaire depuis l'entrée standard
    std::string input;
    std::getline(std::cin, input);

    // Extraction des valeurs login et mot de passe
    std::string login = getQueryParam(input, "login");
    std::string mdp = getQueryParam(input, "mdp");
    ...
    if (checkCredentials(login, mdp)) {
        ...
        // Retourne la page de connexion avec un message d'erreur
        std::cout << "<!DOCTYPE html>\n";
        std::cout << "<html lang=\"en\">\n";
        std::cout << "<head>\n";
        std::cout << "    <meta charset=\"UTF-8\">\n";
        ...
        std::cout << "    <form action=\"traitement.cgi\" method=\"post\">\n";
        ...
        std::cout << "    </form>\n";
        // Affichage du message d'erreur en dessous du formulaire
